...
if [ "${OS}" = "android" ]; then
  EXTRA_ARGS="v8_enable_pointer_compression=false v8_enable_webassembly=false v8_target_cpu=\"${ARCH}\" android_ndk_root=\"${ANDROID_NDK_ROOT}\" android_ndk_version=\"${ANDROID_NDK_VERSION}\" android_ndk_major_version=28"
fi
