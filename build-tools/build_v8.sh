...
if [ "${OS}" = "android" ]; then
  EXTRA_ARGS="v8_enable_pointer_compression=false v8_enable_webassembly=false is_component_build=false use_custom_libcxx=false v8_enable_atomic_object_field_writes=false v8_enable_concurrent_marking=false android_ndk_root=\"${ANDROID_NDK_ROOT}\""
fi
